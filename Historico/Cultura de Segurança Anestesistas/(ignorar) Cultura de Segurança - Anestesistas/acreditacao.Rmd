```{r}
setwd("C:/Users/Gabriel/Onedrive/Documentos/Cultura de Segurança - Anestesistas")
...
df = df %>% filter(quant >= 10)
```


```{r}
hist(df$quant, breaks=25)
table(df$quant)
summary(df$quant)
```
```{r}
...
colunas_pilares <- c("acreditacao", 'Abertura de comunicação', 'Feedback e comunicação sobre erros', 'Frequência de eventos relatados',
...
df_filter = df[, colunas_pilares]
ncol(df_filter)

df_filter$acreditacao = as.factor(df_filter$acreditacao)
ggpairs(df_filter, columns = 2:13, ggplot2::aes(colour=acreditacao))
ggsave("cor.png", height=44, width=44, units="cm", dpi=600)
```
...
for (coluna in lista){
  if (class(df_ano[[coluna]]) == "numeric"){
    ...
    #Teste de Hipotese - Teste t
    ##teste_t = retorne_p(t.test(df_ano[[coluna]]~df_ano$acreditacao, data=df_ano)$p.value)
...
analise_grafica2('Nota Geral == Boa ?', '')
ggsave("Proporção_notas.png", height=9, width=15, units="cm", dpi= 600)
